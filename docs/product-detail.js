...
function renderContributorList(values) {
  const items = Array.isArray(values) ? values.filter((item) => item && (item.name || item.contribution)) : [];
  if (!items.length) return '<p>Not listed</p>';
  return `<div class="vendor-inline-list">${items.map((item) => `<div><strong>${esc(item.name || 'Contributor')}</strong>: ${esc(item.contribution || 'No specific contribution recorded.')}</div>`).join('')}</div>`;
}

function renderSteps(values) {
  const items = Array.isArray(values) ? values.filter(Boolean) : [];
  if (!items.length) return '<p>Not listed</p>';
  return `<div class="vendor-inline-list">${items.map((item, index) => `<div><strong>Step ${index + 1}:</strong> ${esc(item)}</div>`).join('')}</div>`;
}
...
async function initStoryDetail() {
  ...
  try {
    ...
    const aiSummary = story.ai_summary || {};

    root.innerHTML = `<section class="section"><div class="innovation-detail-hero">${story.cover_image_url ? `<img class="innovation-detail-image" src="${esc(story.cover_image_url)}" alt="${esc(story.title)}" referrerpolicy="no-referrer" />` : ''}<div class="innovation-detail-summary"><div class="vendor-result-top"><div><h3>${esc(story.title || 'Untitled story')}</h3><p>${esc(story.person_name || 'Unknown person')} | ${esc(story.place_label || 'Place not listed')}</p></div><span class="admin-badge approved">${esc(story.thematic_area || 'General')}</span></div><p>${esc(story.summary_of_work || story.story_excerpt || 'No saved summary available.')}</p><div><strong>People and Actions</strong>${renderContributorList(aiSummary.contributors)}</div><div><strong>Process Steps</strong>${renderSteps(aiSummary.process_steps)}</div><div><strong>6M Classification</strong>${renderChips(story.six_m_categories, 'innovation-chip')}</div><div><strong>Tags</strong>${renderChips(story.tags)}</div><div class="vendor-detail-grid"><div><h4>Contact Summary</h4><p><strong>Email:</strong> ${esc(story.contact_email || 'Not listed')}</p><p><strong>Phone:</strong> ${esc(story.contact_phone || 'Not listed')}</p><p><strong>Address:</strong> ${esc(story.contact_address || 'Not listed')}</p><p><strong>Published:</strong> ${esc(story.source_published_at ? new Date(story.source_published_at).toLocaleDateString('en-IN', { dateStyle: 'medium' }) : 'Not listed')}</p></div><div><h4>Source Links</h4><p><strong>Better India:</strong> ${story.story_url ? `<a href="${esc(story.story_url)}" target="_blank" rel="noreferrer">View on Better India</a>` : 'Not listed'}</p><p><strong>Person Detail:</strong> <a href="./vendor-detail.html?person=${encodeURIComponent(story.person_name || '')}">Open person profile</a></p><p><strong>Admin Notes:</strong> ${esc(story.admin_notes || 'Not listed')}</p></div></div></div></div></section>`;
  } catch (error) {
